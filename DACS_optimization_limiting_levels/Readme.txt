...
integers limiting the number of levels to use.
...
$ ./test <integer list file>  <outfile>  <max levels>


The input format for the integer list is a binary file containing a 4 bytes field
for each integer.

It saves DACs' representation of the integer list, limiting the 
number of levels to <max levels>, in <outfile>.